...
function makeScatterRenderer(PlotlyComponent, isBubble) {
    ...
    _createClass(Renderer, [{
      ...
      value: function render() {
        ...
        rowKeys.map(function (rowKey) {
          ...
          if (isBubble) data.marker.size = [].concat(_toConsumableArray(data.marker.size), _toConsumableArray(size.map(function (size) {
            return size * Math.pow(100, findY(minSize));
          })));
        });
...
function createPlotlyRenderers(PlotlyComponent) {
  return {
    ...
    'Scatter Chart': makeScatterRenderer(PlotlyComponent, false),
    'Bubble Chart': makeScatterRenderer(PlotlyComponent, true),
    'Multiple Pie Chart': makeRenderer(PlotlyComponent, { type: 'pie', scalegroup: 1, hoverinfo: 'label+value', textinfo: 'none' }, {}, true),
    'Donut Chart': makeRenderer(PlotlyComponent, { hole: 0.8, type: 'pie', scalegroup: 1, hoverinfo: 'label+value', textinfo: 'none' }, {}, true)
  };
}
